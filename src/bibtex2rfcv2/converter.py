...
def extract_ascii(text: str) -> Optional[str]:
    """Extract ASCII version of text by removing non-ASCII characters.
    
    Args:
        text: Input text
        
    Returns:
        ASCII version of text or None if no changes needed
    """
    if not text:
        return None
    ascii_text = re.sub(r'[^\x00-\x7F]+', '', text)
    return ascii_text if ascii_text != text else None
...
def bibtex_entry_to_rfcxml(entry: BibTeXEntry) -> str:
    ...
    if not authors:
        # Add a dummy author if still empty
        authors = ["Unknown"]

    for author in authors:
        ascii_fullname = extract_ascii(author)
        front.authors.append(Author(fullname=author, ascii_fullname=ascii_fullname))

    # Add abstract and note if present
    ...
    # Add date information as seriesInfo only
    ...
    # Add series info based on entry type
    ...
    # Add all other fields as seriesInfo
    handled_fields = {
    ...
    # Map for well-known fields to expected case
    field_case_map = {
        "url": "URL",
        "doi": "DOI",
    ...
    for field, value in entry.fields.items():
        if field not in handled_fields and value:
            # Use expected case for well-known fields
            field_name = field_case_map.get(field.lower())
            if not field_name:
                # Convert field name to proper case
